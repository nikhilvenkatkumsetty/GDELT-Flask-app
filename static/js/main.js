...
const image = require("../data/LOOKUP-IMAGETAGS.json");
const theme = require("../data/LOOKUP-GKGTHEMES.json");
const lang = require("../data/LOOKUP-LANGUAGES.json");
const count = require("../data/LOOKUP-COUNTRIES.json");
const ADM1 = require("../data/LOOKUP-ADM1.json");
const sort = require("../data/lookup-sort.json");
const domain = require("../data/lookup-domain.json");
const mode = require("../data/lookup-mode.json");
const timeline = require("../data/lookup-timeline.json");
const tv = require("../data/lookup-tv.json");
const format = require("../data/lookup-format.json");
const geo_mode = require("../data/lookup-geo_mode.json");
const geo_format = require("../data/lookup-geo_format.json");
const stations = require("../data/LOOKUP-STATIONS.json");

// load selection option sets and append as options to DOM. Adds title tooltips to new elements
load_menu_data(image, ['#imagetag'], [7], ['Every image processed by GDELT is assigned one or more topical tags from a universe of more than 10,000 objects and activities recognized by Google']);
load_menu_data(theme, ['#theme'], [7], ['Searches for any of the GDELT Global Knowledge Graph (GKG) Themes. GKG Themes offer a powerful way of searching for complex topics, since there can be numerous different phrases or names under a single heading. Key in likely relevant themes to find matching options. Words on the left denote the semantic hierarchy.']);
load_menu_data(lang, ['#searchlang','#sourcelang'], [1,7], ['','Language(s) of the content you are searching for. GDELT handles the interpretation']); // searchlang deprecated, but new local lang API anticipated
load_menu_data(count, ['#sourcecountry','#geolocationcc'], [7,7], ['Country or countries where the target content has originated','Specify country of media mentions']);
load_menu_data(ADM1, ['#geolocationadm1'], [7], ['Specify ADM1 (top sub-national) geographical region of media mentions']);
load_menu_data(sort, ['#sort'], [1], ['By default results are sorted by relevance. You can also sort by date or article tone instead']);
load_menu_data(domain, ['#domain'], [7], ['Web domain of target content - e.g. "cnn.com"']);
load_menu_data(mode, ['#contentmode'], [1], ['GDELT modes for investigating source content']);
load_menu_data(timeline, ['#timelinemode'], [1], ['GDELT modes for investigating trends over time']);
load_menu_data(tv, ['#tvmode'], [1], ['GDELT modes for investigating television trends']);
load_menu_data(format, ['#format'], [1], ['Data formats for data export']);
load_menu_data(geo_mode, ['#geomode'], [1], ['GDELT modes for investigating geographical references within the content. This use GDELT\'s 7 day GEO API.']);
load_menu_data(geo_format, ['#geoformat'], [1], ['Data formats for data export']);
...
load_menu_data(stations, ['#network'], [7], ['TV networks - national and international.']);
